...
module Searches
  class SearchService < Searches::BaseSearchService
    ...
    def perform
      ...
      if search_type('article', @params[:selected_types])
        article_search = Articles::SearchService.new(
          ...
          where:            {
                              ...
                              tag_ids:   @params[:tag_ids] ? @params[:tag_ids].first.split(',') : nil
      ...
      begin
        searches = Searchkick.multi_search([article_search&.perform&.result, tag_search&.perform&.result, topic_search&.perform&.result].compact)
