...
class Tag < ApplicationRecord
  ...
  def name_visibility
    ...
    if self.only_me? && Tag.where('visibility = 1 AND user_id = :user_id AND lower(name) = :name', user_id: self.user_id, name: self.name.downcase.to_s).exists?
      errors.add(:name, I18n.t('activerecord.errors.models.tag.already_exist'))
    elsif self.everyone? && Tag.where('visibility = 0 AND lower(name) = :name', name: self.name.downcase.to_s).exists?
