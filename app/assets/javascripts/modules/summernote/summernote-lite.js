...
// CONCATENATED MODULE: ./src/js/base/core/dom.js
                ...
                function wrap(node, wrapperName) {
                    ...
                    var wrapper = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<' + wrapperName + '>')[0]; // ### : Check parent presence
...
// CONCATENATED MODULE: ./src/js/base/core/async.js
                ...
                function createPicture(url, param, imageId, srcsets) {
                    return external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default.a.Deferred(function (deferred) {
                        var $picture = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<picture>');
                        $picture.attr('data-id', imageId);

                        for (var srcsetIndex = 0; srcsetIndex < srcsets.length; srcsetIndex++) {
                            if (srcsets[srcsetIndex].maxWidth) {
                                $picture.append(external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<source>').attr('media', '(max-width: ' + srcsets[srcsetIndex].maxWidth + 'px)').attr('data-srcset', srcsets[srcsetIndex].url));
                            } else {
                                $picture.append(external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<source>').attr('data-srcset', srcsets[srcsetIndex].url));
                            }
                        }

                        var $img = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<img>').addClass('lazyload').attr('data-src', url).attr('data-id', imageId);
                        $picture.append($img);
...
// CONCATENATED MODULE: ./src/js/lite/ui.js
                ...
                var popover = renderer["a" /* default */].create(['<div class="note-popover bottom">', '<div class="note-popover-arrow"/>', '<div class="note-popover-content note-children-container"/>', '</div>'].join(''), function ($node, options) {
