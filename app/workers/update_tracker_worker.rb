...
class UpdateTrackerWorker
  ...
  def count_visits(tracked_class, element_value)
    Ahoy::Event.where(name: 'page_visit').where("properties->>'#{tracked_class.downcase}_id' = ?", element_value.to_s).joins(:visit).merge(Ahoy::Visit.validated.external).distinct.count(:visit_id)
  end
